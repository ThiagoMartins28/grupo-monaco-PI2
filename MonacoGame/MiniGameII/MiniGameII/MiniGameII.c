...
int main() {
    ...
    // Guia
    ALLEGRO_BITMAP* guia = al_load_bitmap("./img/guia.png");
    ...
    // Adicionando variável para controle de pausa
    bool paused = false;
    ...
    while (running) {
        ...
        if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
            ...
            if (event.mouse.x >= book_button.button_x && event.mouse.x <= book_button.button_x + book_button.button_width &&
                ...
                if (paused) {
                    // Desenha a tela de pausa (quadrado branco cobrindo 80% da tela)
                    al_draw_filled_rectangle(0.1 * al_get_display_width(display), 0.1 * al_get_display_height(display),
                        0.9 * al_get_display_width(display), 0.9 * al_get_display_height(display),
                        al_map_rgb(255, 255, 255));

                    int guia_width = al_get_bitmap_width(guia);
                    int guia_height = al_get_bitmap_height(guia);

                    al_draw_scaled_bitmap(
                        guia,
                        0, 0, guia_width, guia_height, // Dimensões originais
                        (1280 - 1000) / 2, // Centraliza horizontalmente
                        (720 - 550) / 2,  // Centraliza verticalmente
                        1000, // Largura fixa
                        550, // Altura fixa
                        0    // Flags
                    );
                    ...
                    //DESENHAR AS POÇÔES AQUI.
                }
            }
        }
        

        if (!paused) {  // Se o jogo não estiver pausado, continua a execução normal
            ...
            // Estado atual: primeira fase do jogo
                ...
                if (event.type == ALLEGRO_EVENT_TIMER) {
                    ...
                    if (is_in_drop_zone && potion_is_dropped) {
                        if (dropped_item_count >= 2) {                          
                            int index1 = dropped_items[dropped_item_count - 2].potion_type;
                            int index2 = dropped_items[dropped_item_count - 1].potion_type;
                            ...
                            if (total_mistura < MAX_MISTURA) {
                                ...
                                if (dropped_item_count >= 3) {
                                    int index3 = dropped_items[dropped_item_count - 3].potion_type;
                                    if (combina_3(index1, index2, index3, 1, 3, 5)) {
